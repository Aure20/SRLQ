...
from exllamav2.conversion.measure import test_error
import torch
...
def quant_linear(job: dict,
    ...
    #Add matrix information to the strat
    
    qparams.update(
        {
            f"{matrix_key}_h_incoherence": lq.incoherence_H, 
            f"{matrix_key}_w_incoherence": lq.incoherence_W,
            f"{matrix_key}_ub": lq.upper_bound,
            **dict(zip([f"{matrix_key}_err_1", f"{matrix_key}_err_5", f"{matrix_key}_err_10", f"{matrix_key}_distortion"], lq.quant_error()))
        }
    )
    ...
    source.linear.weight.data = recons_w.T.to(r_device)
...
def quant_attn(job, module, hidden_states, target_states, quantizers, attn_params, strat):
    ...
    accuracy,mse_error = test_error(module, hidden_states, target_states, None, attn_params)
    strat['accuracy'] = accuracy
    strat['mse_error'] = mse_error
    ...
    quant_linear(job, module.q_proj, quantizers["q_proj"], strat["q_proj"], "q") 
    quant_linear(job, module.k_proj, quantizers["k_proj"], strat["k_proj"], "k")
    del quantizers[f"k_proj"]
    quant_linear(job, module.v_proj, quantizers["v_proj"], strat["v_proj"], "v")
    del quantizers[f"v_proj"]
    quant_linear(job, module.o_proj, quantizers["o_proj"], strat["o_proj"], "o")
...
def quant_mlp(job, module, hidden_states, target_states, quantizers, attn_params, strat, reuse_h_up_proj = None):
    ...
    if has_mlp:
        ...
        quant_linear(job, module.gate_proj, quantizers["gate_proj"], strat["gate_proj"], "gate")
        del quantizers[f"gate_proj"]

    gc.collect()
    torch.cuda.empty_cache()

    quant_linear(job, module.up_proj, quantizers["up_proj"], strat["up_proj"], "up")
    ...
    quant_linear(job, module.down_proj, quantizers["down_proj"], strat["down_proj"], "down")
...
def quant_moe_mlp(job, module, hidden_states, target_states, quantizers, attn_params, strat):
    ...
    for i in range(num_experts):
        quant_linear(job, module.w1[i], quantizers[f"w1.{i}"], strat["w1"], f"w2.{i}")
        del quantizers[f"w1.{i}"]
        quant_linear(job, module.w3[i], quantizers[f"w3.{i}"], strat["w3"], f"w2.{i}")
        del quantizers[f"w3.{i}"]
        quant_linear(job, module.w2[i], quantizers[f"w2.{i}"], strat["w2"], f"w2.{i}")
...
def quant_lm_head(job, module, hidden_states, quantizers, attn_params, rtn = False):
    ...
    quant_linear(job, module, q, qp.get_dict(), 'head', drop = True, rtn = rtn)
...
#
#     a_batch = torch.empty((rows * cols, dim), dtype = torch.float, device = f"cuda:{int(os.environ['LOCAL_RANK'])}")
#     b_batch = torch.empty((rows * cols, dim), dtype = torch.float, device = f"cuda:{int(os.environ['LOCAL_RANK'])}")
#
#     r = 0
#     for state, target_state in zip(states, target_states):
#         a = norm.forward(state.to(f"cuda:{int(os.environ['LOCAL_RANK'])}"))
#         b = norm.forward(target_state.to(f"cuda:{int(os.environ['LOCAL_RANK'])}"))
...
def quant(job, save_fn, model):
    ...
    while True:
        ...
        for i in range(len(hidden_states)):
            ...
            x = hidden_states[i].to(f"cuda:{int(os.environ['LOCAL_RANK'])}")
        ...
        if mode == "self_attn": 
            quant_attn(job, module, hidden_states, target_states, quantizers, attn_params, job["strategy"][module.key + "." + mode])

        if mode == "mlp": 
            quant_mlp(job, module, hidden_states, target_states, quantizers, attn_params, job["strategy"][module.key + "." + mode])

        if mode == "block_sparse_moe": 
            quant_moe_mlp(job, module, hidden_states, target_states, quantizers, attn_params, job["strategy"][module.key + "." + mode])
        ...
        if mode == "parallel_decoder":
            strat_attn = job["strategy"][module.key + ".self_attn"]
            strat_mlp = job["strategy"][module.key + ".mlp"]
        ...
        if mode == "linear":
            ...
            module.linear.weight.data = module.linear.weight.data.to(f"cuda:{int(os.environ['LOCAL_RANK'])}")
        ...
        for i in range(len(hidden_states)):
            ...
            if mode != "linear":
                ...
                output_ref = target_states[i].to(f"cuda:{int(os.environ['LOCAL_RANK'])}")
                ...
                target_ids = cal_ids[i:i+1, 1:].to(f"cuda:{int(os.environ['LOCAL_RANK'])}")
    ...
    """
    return perplexity, job['strategy']
