...
def test_error(module, hidden_states, target_states, cache, attn_params):
    ...
    for x, xref in zip(hidden_states, target_states):
        ...
        rfn_count += 1
        mse_sum += torch.nn.functional.mse_loss(xtest, xref)

    mse_error = mse_sum.item() / rfn_count
    return max(1e-6, 1 - (rfn_sum.item() / rfn_count)), mse_error
...
def measure_attn(module, hidden_states, target_states, quantizers, cache, attn_params, keep_q = False):
    ...
    for (q, k, v, o) in qmaps:
        ...
        accuracy, mse_error = test_error(module, hidden_states, target_states, cache, attn_params)
        ...
        r = {
            ...
            "q_proj": {**qjobs[0][q].get_dict(), **{
                "q_bits": bits_q[q],
                "q_h_incoherence": quantizers["q_proj"].incoherence_H,
                "q_w_incoherence": quantizers["q_proj"].incoherence_W,
                "q_ub": quantizers["q_proj"].upper_bound,
                **dict(zip(["q_err_1", "q_err_5", "q_err_10", "q_distortion"], quantizers["q_proj"].quant_error()))
            }},
            "k_proj": {**qjobs[1][k].get_dict(), **{
                "k_bits": bits_k[k],
                "k_h_incoherence": quantizers["k_proj"].incoherence_H,
                "k_w_incoherence": quantizers["k_proj"].incoherence_W,
                "k_ub": quantizers["k_proj"].upper_bound,
                **dict(zip(["k_err_1", "k_err_5", "k_err_10", "k_distortion"], quantizers["k_proj"].quant_error()))
            }},
            "v_proj": {**qjobs[2][v].get_dict(), **{
                "v_bits": bits_v[v],
                "v_h_incoherence": quantizers["v_proj"].incoherence_H,
                "v_w_incoherence": quantizers["v_proj"].incoherence_W,
                "v_ub": quantizers["v_proj"].upper_bound,
                **dict(zip(["v_err_1", "v_err_5", "v_err_10", "v_distortion"], quantizers["v_proj"].quant_error()))
            }},
            "o_proj": {**qjobs[3][o].get_dict(), **{
                "o_bits": bits_o[o],
                "o_h_incoherence": quantizers["o_proj"].incoherence_H,
                "o_w_incoherence": quantizers["o_proj"].incoherence_W,
                "o_ub": quantizers["o_proj"].upper_bound,
                **dict(zip(["o_err_1", "o_err_5", "o_err_10", "o_distortion"], quantizers["o_proj"].quant_error()))
            }}
        }
    ...
    if has_gate:
        ...
        for (g, u, d) in qmaps:
            ...
            r = {
                ...
                "mse": mse_error,
                "total_bits": total_bits, 
                "gate_proj": {**qjobs[0][u].get_dict(), **{
                    "gate_bits": bits_g[g],
                    "gate_h_incoherence": quantizers["gate_proj"].incoherence_H, 
                    "gate_w_incoherence": quantizers["gate_proj"].incoherence_W,
                    "gate_ub": quantizers["gate_proj"].upper_bound,
                    **dict(zip(["gate_err_1", "gate_err_5", "gate_err_10", "gate_distortion"], quantizers["gate_proj"].quant_error()))
                }},
                "up_proj": {**qjobs[1][u].get_dict(), **{
                    "up_bits": bits_u[u],
                    "up_h_incoherence": quantizers["up_proj"].incoherence_H, 
                    "up_w_incoherence": quantizers["up_proj"].incoherence_W,
                    "up_ub": quantizers["up_proj"].upper_bound,
                    **dict(zip(["up_err_1", "up_err_5", "up_err_10", "up_distortion"], quantizers["up_proj"].quant_error()))
                }},
                "down_proj": {**qjobs[2][d].get_dict(), **{
                    "down_bits": bits_d[d],
                    "down_h_incoherence": quantizers["down_proj"].incoherence_H,
                    "down_w_incoherence": quantizers["down_proj"].incoherence_W,
                    "down_ub": quantizers["down_proj"].upper_bound,
                    **dict(zip(["down_err_1", "down_err_5", "down_err_10", "down_distortion"], quantizers["down_proj"].quant_error()))
                }}
            }
...
def measure_parallel_decoder(module, hidden_states, target_states_attn, target_states_mlp, quantizers, cache, attn_params):
    ...
    for i in range(len(hidden_states)):
        hidden_states[i] = hidden_states[i].to(f"cuda:{torch.cuda.current_device()}")
...
def measure_quant(job, save_fn, model, hidden_state_offload_layers):
    ...
    with safe_open(states_filename, framework = "pt", device = "cpu") as f:
        for i, k in enumerate(sorted(f.keys())):
            t = f.get_tensor(k)
            hidden_states.append(t.to(f"cuda:{torch.cuda.current_device()}") if i < hidden_state_offload_layers else t)
    ...
    while True:
        ...
        for i in range(len(hidden_states)):
            ...
            x = hidden_states[i].to(f"cuda:{torch.cuda.current_device()}")
            ...
            target_device = f"cuda:{torch.cuda.current_device()}" if i < hidden_state_offload_layers else "cpu"
