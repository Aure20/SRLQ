...
import numpy as np 
def optimize(job, save_fn, model, opt_params):
    ...
    #--------------------------------------------------------
    #Apply parameters to the accuracy score
    accuracies = []
    incoherences_h = []
    incoherences_w = []
    bounds = []
    ...
    for k,v in job["measurement"].items(): 
        if v is None:
            break
        for config in v: 
            for key,value in config.items():
                if 'proj' in key:
                    layer = key.split('_')[0]
                    incoherences_h.append(value[layer+'_h_incoherence'])
                    incoherences_w.append(value[layer+'_w_incoherence'])
                    bounds.append(value[layer+'_ub'])
                    distortions.append(value[layer+'_distortion'])
                    err_1.append(value[layer+'_err_1'])
                    err_5.append(value[layer+'_err_5'])
                    err_10.append(value[layer+'_err_10'])
                    accuracies.append(config['accuracy']) 
    ...
    def normalize(arr):
        ...
        with warnings.catch_warnings():
            ...
            try:
                arr = (arr - arr.min()) / (arr.max() - arr.min()) #If all values are equal then it returns an error
            except RuntimeWarning:  
                return arr  
        return arr

    adjusted_accuracies = np.stack((normalize(accuracies),normalize(incoherences_h),normalize(incoherences_w),normalize(bounds),normalize(distortions), normalize(err_1), normalize(err_5), normalize(err_10)))
    
    adjusted_accuracies = normalize(opt_params@adjusted_accuracies )
     

    index = 0 #Store the current index
    for k,v in job["measurement"].items(): 
        if v is None:
            break
        for config in v: 
            if 'mlp' in k:
                config['accuracy'] = adjusted_accuracies[index:index+3].mean() #Take the mean of the values in the accuracy slot
                index += 3
            elif 'self_attn' in k:
                config['accuracy'] = adjusted_accuracies[index:index+4].mean() #Take the mean of the values in the accuracy slot
                index += 4
    #--------------------------------------------------------
