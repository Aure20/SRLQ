...
class AdaptiveGPTQ:
    ...
    incoherence_W: float = -1
    incoherence_H: float = 0 
    fro_x : float = -1
    fro_w : float = 0 
    upper_bound: float = 0
    in_dim: int = 0 
    ...
    def __init__(self,
        ...
        self.quant_device = self.device
        self.hessian_device = self.device
    ...
    def configure(self,
        ...
        assert sum(bits_prop) == 1, bits_prop
        ...
        R = sum([a*b for (a,b) in zip(bits_prop, bits)]) 
        centered_weights = (self.layer.weight.data.T-self.layer.weight.data.T.mean(dim=-2,keepdims=True)) #(Should I move it as well to GPU?)
        self.fro_w = torch.norm(centered_weights,'fro').item()**2
        assert self.fro_x != -1
        self.upper_bound = ((self.fro_x*self.fro_w)/self.rows)*((1/(2**(2*R)-1))+0.1)+(self.rows**(-8))*self.columns*self.in_dim  
    ...
    def add_batch(self, inputs):
        ...
        with torch.inference_mode():
            ...
            # dim = inputs.shape[-1]
            # inputs = inputs.view((-1, dim)).float().T.to(self.device)
            # ns = 1
            #
            ...
            # inputs = inputs.view((-1, inputs.shape[-1])).float().T.to(self.device)
            ...
            #Accumulate frobenius norms for bound 
            centered_inputs = inputs.permute(0,1,2) - inputs.permute(0,1,2).mean(dim=-2,keepdims=True) 
            self.fro_x += torch.norm(centered_inputs,'fro').item()**2
            self.in_dim += inputs.shape[-2]
            centered_inputs = None 

            self.num_batches += 1
            inputs = inputs.view((-1, inputs.shape[-1])).float().to(self.device)
    ...
    def prepare(self, no_h_inv = False):
        ...
        with torch.inference_mode():
            ...
            if self.hessian.numel() > 6e8:
                ...
                hessian = hessian.to(self.device)
            else:
    ...
    def reuse_h(self, other):
        ...
        with torch.inference_mode():
            #Get incoherence in case it has not been computed yet:
            if self.incoherence_W == -1:
                self.incoherence_W = (torch.max(torch.abs(self.layer.weight.data))*math.sqrt(self.columns*self.rows)/torch.norm(self.layer.weight.data, 'fro')).item()
                self.incoherence_H = other.incoherence_H

            #Same for the upper bound
            if self.fro_x == -1:
                self.fro_x = other.fro_x 
    ...
    def quantize_rtn_inplace(self, keep_qweight = False, apply = False):
        ...
        with torch.inference_mode():
            ...
            self.invperm = torch.argsort(self.perm).to(self.device)

            # Store scales

            self.scale = torch.stack(scale, dim = 0).to(self.device)
            self.qscale = torch.stack(qscale, dim = 0).to(self.device)
            self.qscale_max = qscale_max.to(torch.float16).to(self.device)
            self.qgroups = torch.tensor(qgroups, dtype = torch.short).to(self.device)
    ...
    def quantize(self, keep_qweight = False, apply = False):
        ...
        with torch.inference_mode():
            ...
            self.quant = self.quant.to(self.device)
            self.invperm = torch.argsort(self.perm).to(self.device)
    ...
    def quant_error(self):
        ...
        with torch.inference_mode():
            ...
            dist = diff@diff.T
            distortion = torch.trace(dist).item()/(self.columns*self.rows)
            return mat_error_1, mat_error_5, mat_error_10, distortion
    ...
    def pack(self, key, qparams):
        ...
        self.qgroups = self.qgroups.to(self.device)
    ...
    def get_incoherence(self): 
        _, Q = torch.linalg.eigh(self.hessian) #Faster eigendecomposition for symmetrical matrices 
        self.incoherence_H = (torch.max(torch.abs(Q))*math.sqrt(Q.shape[0])).item()
        self.incoherence_W = (torch.max(torch.abs(self.layer.weight.data))*math.sqrt(self.columns*self.rows)/torch.norm(self.layer.weight.data, 'fro')).item()
